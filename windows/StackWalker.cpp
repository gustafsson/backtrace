...
public:
  ...
  BOOL Init(LPCSTR szSymPath)
  {
    ...
    {
      wcscat_s(szTemp, 4096, L".local");
      if (GetFileAttributes(szTemp) == INVALID_FILE_ATTRIBUTES)
      {
        // ".local" file does not exist, so we can try to load the dbghelp.dll from the "Debugging Tools for Windows"
        if (GetEnvironmentVariable(L"ProgramFiles", szTemp, 4096) > 0)
        {
          wcscat_s(szTemp, 4096, L"\\Debugging Tools for Windows\\dbghelp.dll");
        ...
        {
          wcscat_s(szTemp, 4096, L"\\Debugging Tools for Windows 64-Bit\\dbghelp.dll");
          if (GetFileAttributes(szTemp) != INVALID_FILE_ATTRIBUTES)
          {
            m_hDbhHelp = LoadLibrary(szTemp);
          }
